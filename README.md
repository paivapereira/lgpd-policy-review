...
- **Robustez a modelo (indício exploratório)** — ablação por estágio na fase de QA mediu dependência residual de modelo localizada no Classifier: com Classifier-Haiku o veredito de violação oscila; subindo-o a Sonnet o falso-negativo observado cai a 0/10. Não conclusiva (single-clause POL-005, K=10, teto IC95% ~30%); ver [`docs/eval/model-robustness-ablation.md`](docs/eval/model-robustness-ablation.md).
