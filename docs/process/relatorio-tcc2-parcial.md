<!--
RELATÓRIO TÉCNICO DE TCC2 — VERSÃO PARCIAL (fonte Markdown)
Este arquivo é a fonte em Markdown do relatório técnico parcial. O entregável institucional formatado
...
**Modalidade:** Relatório Técnico de TCC2 (versão parcial)
...
- Quadro 3 — Cronograma consolidado
...
> *Nota de estado (relatório parcial).* À data deste relatório, possuem especificação dedicada e revisada os subagentes Triager, Detector, Classifier e Reporter, além do coordenador. A especificação do Matcher encontra-se em elaboração; seu contrato está, por ora, definido na etapa correspondente do coordenador (`coordinator.md` §3.4). As descrições a seguir refletem esse estado e serão consolidadas quando as especificações dedicadas forem concluídas.
...
A **integração contínua** é provida pelo GitHub Actions, que dispara o sistema a cada *pull request*, e a **validação empírica** apoia-se no *framework* Inspect AI, utilizado para executar o *benchmark* sintético de trechos rotulados descrito na seção seguinte (RNF-001).
...
| Inspect AI | — | *Framework* de validação empírica | RNF-001 |

Fonte: Autoria própria (2026).

## 2.5 AVALIAÇÃO

> *Nota de estado (relatório parcial).* O método de avaliação descrito nesta seção — o portão de verificação em dois escopos e o desenho do *benchmark* sintético — é definitivo e não deve mudar até a entrega final. Os resultados, contudo, são parciais: à data deste relatório, apenas os componentes de fundação foram validados, e a avaliação empírica do sistema integrado é etapa ainda pendente, detalhada ao final da seção.
...
A **validação empírica** do comportamento do sistema apoia-se em um *benchmark* sintético de trechos de código rotulados por veredito esperado, executado por meio do *framework* Inspect AI. Cada trecho é construído para acionar uma situação específica e vem anotado com o veredito que o sistema deveria produzir para a cláusula aplicável, permitindo confrontar a saída observada com a esperada. Dois critérios de validação global orientam essa frente (`DESIGN.md`). O primeiro é a verificação de ponta a ponta: executar o sistema sobre um *pull request* sintético que contém uma violação plantada — por exemplo, a persistência de um CPF sem anonimização — e obter um Report JSON com o achado correto, incluindo a referência à cláusula, o veredito, a evidência e a trinca de proveniência. O segundo é o teste de generalização, que materializa a propriedade de independência jurisdicional (RF-008): substituir a Política e os vocabulários da LGPD por equivalentes do GDPR, reexecutar o mesmo *pull request* e obter um Report válido sob o novo *framework*, sem qualquer alteração no código do sistema.
...
Quanto ao **estado de execução** à data deste relatório parcial, a verificação aplica-se de forma desigual às camadas do sistema. Os dois servidores MCP — `policy-reader` e `semgrep-runner` — encontram-se implementados e aprovados em seus respectivos portões de marco, este último com validação empírica registrada. A Camada 2 — coordenador, subagentes e a ferramenta `emit_report` — encontra-se em especificação e início de implementação, e a Camada 3 — integração com o GitHub Actions — está planejada; a validação empírica de ponta a ponta do sistema integrado, bem como a construção do *benchmark*, são as etapas seguintes do cronograma. Assim, os resultados de avaliação disponíveis até o momento atestam o comportamento dos componentes de detecção e de acesso à Política, e não ainda o da *pipeline* multiagente integrada.

## 2.6 CRONOGRAMA

O cronograma original, registrado na proposta de orientação (`proposta` §9), previa seis semanas encerrando em 15 de junho de 2026. A fase de especificação estendeu-se além do previsto — em razão do rigor adotado na redação das especificações e nas revisões cruzadas —, resultando em um deslocamento de aproximadamente uma semana e meia em relação ao plano inicial. O cronograma foi, por isso, consolidado conforme o Quadro 3, mantida a data de entrega. As fases referenciam o ciclo do *Spec-Driven Development* descrito na seção 2.1.

À data deste relatório parcial, encontram-se concluídos os dois servidores MCP (Milestones A e B, ambos com portão aprovado) e parte das especificações da camada multiagente (coordenador, Reporter, Triager, Classifier e Detector); resta a especificação do subagente Matcher e a implementação da camada multiagente, do mecanismo de integração contínua e a validação empírica do sistema integrado.
...
| 29/mai – 02/jun | Esqueleto do coordenador, da ferramenta `emit_report` e da *GitHub Action*; início da redação do relatório e da implementação dos subagentes | *Specify* / *Implement* | Em andamento |
| 03 – 09/jun | Especificação do subagente Matcher; coordenador consolidado; implementação dos subagentes; *pull requests* sintéticos | *Specify* / *Implement* | Planejado |
| 10 – 13/jun | Implementação de `emit_report` e da *GitHub Action* funcional; primeira validação de ponta a ponta e refinamento | *Implement* / *Validate* | Planejado |
| 14/jun | Validação final; figuras, tabelas e formatação; reserva (*buffer*) para imprevistos | *Validate* | Planejado |
| 15/jun | Revisão final e entrega | — | Planejado |

Fonte: Autoria própria (2026).

A redação do relatório ocorre em paralelo às demais frentes ao longo do período, e não como etapa final concentrada, para reduzir o risco de acúmulo na última semana. O dia 14 de junho opera como reserva: em execução conforme o plano, destina-se à finalização; havendo deslocamento na semana de validação, absorve o atraso sem comprometer a entrega. O plano prevê, ainda, uma postura de contingência por redução de escopo — detalhada como trabalho futuro no Capítulo 3 —, segundo a qual, não se concluindo a validação integrada no prazo, prioriza-se a finalização do relatório e a entrega de uma prova de conceito com cobertura reduzida.

---

# 3 CONSIDERAÇÕES PARCIAIS

Este trabalho propôs-se a projetar e implementar um sistema de *code review* automatizado que verifica a conformidade do tratamento de dados pessoais com uma Política versionada, demonstrando a viabilidade dessa Política como artefato declarativo independente tanto do mecanismo que a interpreta quanto da jurisdição que codifica. À data deste relatório parcial, o trabalho encontra-se em fase de execução, e as considerações aqui registradas são, portanto, provisórias: consolidam o que foi alcançado e aprendido até o momento, e delimitam o que resta para a entrega final.

No plano do que foi alcançado, a arquitetura em três camadas está integralmente especificada, e as camadas de fundação estão implementadas e validadas. A Política versionada (Camada 1) foi modelada como artefato declarativo de primeira classe, com identidade em três eixos independentes e vocabulários jurisdicionais externalizados como dado, o que sustenta tanto a composição de múltiplas fontes de obrigação dentro de uma jurisdição quanto a substituição integral da jurisdição sem alteração de código. Os dois servidores que dão acesso à Política e à detecção sintática (parte da Camada 2) foram implementados e aprovados em validação, incluindo os reconhecedores de identificadores brasileiros — o diferencial técnico do trabalho frente às ferramentas internacionais existentes. A camada multiagente — coordenador e cinco subagentes especializados — encontra-se parcialmente especificada, restando a conclusão da especificação de um subagente, a implementação integrada e a validação empírica de ponta a ponta. A propriedade central que o trabalho pretende sustentar — a independência da Política em relação ao mecanismo e à jurisdição — está, assim, estabelecida no nível de projeto, e sua demonstração empírica integrada é a etapa imediatamente seguinte.
...
Além das etapas restantes do produto mínimo viável — a implementação da camada multiagente, a integração com o GitHub Actions e a validação empírica do sistema integrado —, o trabalho delimita explicitamente um conjunto de evoluções fora do escopo atual, registradas como decisões de projeto (`architecture-overview` §7.3). São elas: a classificação de severidade dos achados; um subagente proponente de correção; o bloqueio condicional de *merge*, condicionado à validação empírica de uma taxa de falsos positivos defensável em base de código real; um mapa de dados longitudinal cruzando informações entre *pull requests*; a cobertura de dimensões adicionais da LGPD além de consentimento e anonimização — transferência internacional, retenção, direitos do titular, dados de menores e tratamento compartilhado; conjuntos de regras de detecção por cliente; a otimização do modelo por etapa, após a validação funcional; e a construção do *benchmark* sintético completo, com portão de marco quantitativo e métricas de precisão e revocação. A delimitação dessas fronteiras protege a defesa do trabalho de questionamentos que ele não se propôs a responder, e protege a evolução posterior de promessas que o produto mínimo viável não assumiu.
...
- **APÊNDICE D — Decisões arquiteturais** (`docs/adr/`): registros de decisão arquitetural (ADR-0001 a ADR-0010 e posteriores), com contexto, decisão e consequências.
- **APÊNDICE E — Especificações dos subagentes** (`docs/specs/subagents/`): especificações do coordenador, do Reporter, do Triager, do Classifier e do Detector; a especificação do Matcher será incorporada quando concluída.
