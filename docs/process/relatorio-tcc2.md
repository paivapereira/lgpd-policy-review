...
A relevância acadêmica, portanto, não reside em nenhuma técnica isolada — análise estática para conformidade, política externalizada do mecanismo, ancoragem de regra a dispositivo legal e revisão de código por agentes são, cada uma, recursos já estabelecidos. Reside na combinação, sem equivalente identificado na literatura, dessas peças em um arranjo específico: uma Política jurídica versionada em três eixos independentes — esquema, conteúdo e jurisdição —, auditável por advogado, aplicada à revisão de *pull requests* sobre código de aplicação, com a jurisdição tratada como dado e apoiada por reconhecedores de identificadores brasileiros. É esse arranjo, e não qualquer um de seus componentes tomado isoladamente, que o trabalho sustenta como contribuição.
