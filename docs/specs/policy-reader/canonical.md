...
Returns the clause object with `clause_id`, `title`, `statutory_reference` (hierarchical structure of law references), `applies_to` (scope of applicability — for substantive clauses, an object with personal_data_categories and operation drawn from the canonical vocabulary; absent on definitional clauses, which carry defines and out_of_scope instead — see Output structure below), `requirements` (numbered sub-items the clause demands), `exceptions` (numbered sub-items that suspend requirements), and `status`.
