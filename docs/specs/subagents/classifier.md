...
**Eixo `tools` — availability, ortogonal à quíntupla.** A query do Classifier declara também `tools=["Read", "Grep"]` no skeleton de `coordinator.md` §3.3. `tools` é eixo **ortogonal** à quíntupla (per `coordinator.md` §2, ratificado em Reporter §1.4 + §1.5): a quíntupla governa *denial-on-miss* (whitelist `allowed_tools` sob `dontAsk`); `tools` governa *availability* — quais built-ins-**base** entram no contexto do modelo (Issue #361 da SDK Python: `allowed_tools` "does not remove tools from Claude's toolset"). `tools` lista apenas os built-ins-base que o Classifier usa (`Read`, `Grep`); **não** lista `ListMcpResourcesTool`/`ReadMcpResourceTool` porque as tools de acesso a resource MCP são disponibilizadas pelo registro do server em `mcp_servers` + entrada em `allowed_tools`, não pelo campo `tools` — a doc oficial do Python SDK é explícita: `allowed_tools` pré-aprova mas *não* controla availability, e tools MCP são habilitadas via `mcp_servers`. É por isso que o Matcher invoca `ReadMcpResourceTool` sob `tools=[]` (coordinator §3.4) sem conflito — precedente consistente. Confirmação empírica deste acoplamento (`tools` restrito + resource MCP funcional) é o Gate resource access de §10.3.
...
> **Forward-reference declarada (obrigação à Matcher spec).** A afirmação "o Matcher é a autoridade downstream sobre membership" é forward-ref à `docs/specs/subagents/matcher.md` (não autorada) — mesmo padrão da DetectorFinding em §2.1. Pré-registra a obrigação: a Matcher spec deve declarar **como** trata um valor recebido fora do vocabulário. Importante: isso **não** exige validação hard em lugar nenhum da cadeia. O design aceita ausência de enforcement programático — um valor fora-de-vocabulário simplesmente não casa nenhuma cláusula no Matcher (`get_clause`/`check_applicability` sobre `applies_to` derivado de vocabulário) → o candidato resolve para `not_applicable`/`indeterminate`, degradação graciosa, não crash. A obrigação à Matcher spec é portanto **documentar essa degradação** (não casa → veredito conservador), não introduzir um `Enum` hard que reintroduziria o anti-padrão removido do Reporter (§4.8). Catalogado como obrigação em §10.5 e como DD-C9 em §8.4/§10.4.
...
| DD-C9   | Postura do Matcher sobre membership recebido (degradação graciosa vs validação) + postura de validação do consumer (`extra=ignore/forbid`) | Matcher spec não autorada; obrigação declarada em §3.3 + §7.1                | Quando a Matcher spec for autorada             |
...
- **ADR-0012 (retroativo, a redigir)** — nuance de scoped access per-server vs per-resource (§3.3).
...
- **Gate resource access** — smoke-test confirmando `ReadMcpResourceTool` resolvendo `policy://vocabularies` (e `policy://examples`) sob a quíntupla com `mcp_servers` não-vazio e tools de ação fora do allowlist (registrar o server não concede as tools). Ainda **não exercitado para o Classifier**; o precedente Matcher é `tools=[]`, shape diferente — não assumir.
...
- **Gate 6** — `sdk_tools_empty_list` (PR #67). Indireto: valida mecânica de `tools` em context restriction (Classifier usa `tools=["Read","Grep"]`, não `[]`).
...
1. **`coordinator.md` §3.3 — patch consolidado da Fase 4 (não só `output_format`/`max_turns`).** O catálogo da rodada anterior listava apenas dois campos; o planning de implementação revelou que o patch ao skeleton §3.3 (linhas 112-147) é maior (G9). Conjunto completo a aplicar como um companion edit:
   - `output_format={"type": "json_schema", "schema": ClassifierOutput.model_json_schema()}` (Branch B; DD-C3). Hoje ausente — verificado verbatim (`output_format` só na linha 72, §3.1 Triager). MC-F adicionou ao Triager mas §3.3 nunca foi tocado.
   - `max_turns` (DD-C6; ver §1.5 — constante generosa, não escala com N).
   ...
   As notas de drift em §1.4 e §1.5 apontam para este item. Verbo: "adicionar/estabelecer", não "confirmar".
...
6. **`docs/specs/subagents/matcher.md` (a autorar) — obrigação de membership + postura de validação (DD-C9).** A Matcher spec deve (a) declarar como trata `operation_type`/`data_categories`/`declared_legal_basis` recebidos fora do vocabulário — design corrente: degradação graciosa (não casa cláusula → `not_applicable`/`indeterminate`), **não** validação hard que reintroduziria o anti-padrão removido do Reporter §4.8; e (b) declarar a postura de validação Pydantic do `structured_context` consumido (`extra="ignore"` vs `"forbid"`), que determina se adicionar campo aqui é minor não-breaking ou breaking ao consumer (ver §7.1). Pré-registrado para não virar surpresa quando a Matcher spec for autorada.
