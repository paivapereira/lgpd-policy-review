...
`max_turns` provisional. Aritmética do trabalho do Detector: 1 chamada `scan_diff` + N leituras `Read` (uma por finding, para `surrounding_context`) + 1 emissão final ≈ **2 + N turns**. Um cap constante implica um **threshold**: PRs com N acima de ~(cap − 2) degradam para `error_max_turns` → `SubagentUnresponsive` (§6.4) → escalação pelo coordinator. **Isso é o backstop desejado** para isolar PRs patológicos, não falha silenciosa nem output parcial. Valor provisional sugerido: **`max_turns=30`** (headroom para ~28 candidatos antes do backstop; o Detector faz 1 `Read`/finding, perfil de turns mais leve que o Classifier que faz `Read`+`Grep`/candidato e usa `20`). **`⚠ DECISÃO`** o valor é judgment do autor — confirmar 30 vs herdar 20 do Classifier; o que **não** vale é deixar implícito que um cap baixo "é seguro" para N grande. Floor calibrado em T11+ contra catálogo de PRs com N grande (measure-before-tune). `max_budget_usd` disponível como cap complementar, não exercitado no MVP. **Nota de drift:** como na Classifier (§1.4), `output_format` e `max_turns` do Detector ainda **não** estão declarados no skeleton de `coordinator.md` §3.3 — companion edit em §10.5 (item alvo: só esses dois, **não** a função `build_detector_prompt`, que já existe — ver §2.3).
...
**Forward-ref ao Reporter (DD-D3, débito pré-registrado).** O Reporter deve aceitar e citar o bloco `ScanProvenance` na proveniência do Report — paralelo aos forward-refs do Matcher. **Adição confirmada** (não "verificar"): o `ReportPayload` atual (`reporter.md` §3) carrega só a Trinca policy/legal (`policy_schema_version`/`policy_version`/`legal_framework`) per-finding; **não** tem `rules_version`/`semgrep_version`/`scan_metadata`. Aceitar/citar `ScanProvenance` requer **minor bump da spec do Reporter + emenda do schema do Report** — companion edit obrigatório antes da implementação T11+ do Reporter integrar a provenance do Detector. Aceitar lista pura (sem envelope) quebraria o pattern de provenance trickle-down e mataria o trinque de auditoria de `semgrep-runner` §6.
...
- **`GIT_REF_NOT_FOUND`**, **`INSUFFICIENT_GIT_HISTORY`** (business, `isRetryable: false`) → escalação; caller precisa corrigir o ref. Report com `run_outcome="error"`.
- **`SEMGREP_BINARY_UNAVAILABLE`** (system, non-retryable), **`SEMGREP_EXECUTION_FAILED`**, **`INVALID_RULE_SET`** → escalação / `run_outcome="error"` conforme retryability da tabela §5 do `semgrep-runner`.
...
- [ ] `scan_diff` `errorCode=GIT_REF_NOT_FOUND` → escalação / `run_outcome="error"`; **não** `findings: []`.
...
- **DD-D3** — *fechada por design.* Envelope `DetectorOutput = {findings, provenance}`; provenance per-scan no nível do envelope (não per-finding); coordinator desempacota (findings→Classifier, provenance→scratchpad+Reporter). Forward-ref ao Reporter. §3.2, §3.5.
...
1. `coordinator.md` §3.3 — declarar `output_format` (envelope `DetectorOutput`) e `max_turns` do Detector no skeleton; **reconciliar a signature de `build_detector_prompt(pr_metadata, triager_output)` já existente** (~linha 94) com o `DetectorInput` notacional desta spec — confirmar que `pr_metadata` carrega `base_ref`/`head_ref`. (Não criar a função.)
...
3. `coordinator.md` — taxonomia de exceções: avaliar `DetectorScanFailed` (ou reuso de `run_outcome="error"`) para erro non-retryable de `scan_diff` (§6.2). **Decisão do coordinator, pendente.**
4. `coordinator.md` — convenção de scratchpad `02-detector.json` confirmada (:84,108,137,172).
5. **Reporter — adição confirmada (companion edit obrigatório).** `ReportPayload` (`reporter.md` §3) carrega só a Trinca policy/legal; **não** tem scan provenance. Aceitar/citar `ScanProvenance` requer **minor bump da spec do Reporter + emenda do schema do Report**, antes da implementação T11+ do Reporter integrar a provenance do Detector (§3.5).
