...
**Rename `_format_first_stat_ref` → `_format_stat_ref` em `tools.py`.** Helper é genérico (faz unpack de qualquer `StatutoryReferenceEntry`); o nome "first" é vestigial do uso histórico em T02a, onde a função renderizava a primeira entrada de `clause.statutory_reference` em `_render_clause_text`. Pós-T04, o helper tem 3 call sites consumindo entries arbitrários (não apenas o primeiro), tornando o nome enganoso. Aplicação do PR sequencing pattern (`.claude/rules/git-conventions.md`): rename de helper compartilhado é cleanup separado, despachado em PR `chore/rename-format-stat-ref` pós-T04 para preservar blame auditability per PR. Sites a atualizar no rename: definição em `tools.py:_format_first_stat_ref`, call em `_verdict_for_control` (linha ~291), call em `_render_clause_text` (linha ~528), call novo em `tools.get_catalog` introduzido por T04 (article_sources_summary list comprehension).

**Sync `docs/specs/policy-reader/canonical.md` §3.1 sobre shape de `article_sources_summary`.** Canonical §3.1 prescreve a presença do campo mas não a forma; tasks.md T04 nota explicitamente que é Code-decision (linha 187). T04 implementou lista de strings renderizadas via `_format_first_stat_ref` (DD-T04-1), uma string por entrada de `clause.statutory_reference`, consistente com o formatter compartilhado usado por `content[0].text` de tools que citam artigo. Sync canonical §3.1 documenta essa decisão como contrato observável. PR de housekeeping pós-T04 — combinável com o item 2 (canonical sync de `evidence`/`reason`) e com o rename do helper em PR única `chore/housekeeping-post-t04`, ou despachados separados conforme preferência de scope discipline.
